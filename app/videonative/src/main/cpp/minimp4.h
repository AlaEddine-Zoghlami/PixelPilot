...
#define HEVC_NAL_CRA_NUT 21
     ...
     */
    MP4D_file_offset_t MP4D_frame_offset(
...
enum
{
    ...
    BOX_stik = FOUR_CHAR_INT(
        ...
        't',
        'i',
        'k'),  // stik (byte)  0 = Movie   1 = Normal  2 = Audiobook  5 = Whacked Bookmark  6 = Music Video  9 = Short
               // Film  10 = TV Show  11 = Booklet  14 = Ringtone
...
static int minimp4_vector_grow(minimp4_vector_t* h, int bytes)
{
    ...
    if (new_size < h->capacity + bytes) new_size = h->capacity + bytes + 1024;
...
{
    unsigned char* p;
    if (!h->data && !minimp4_vector_init(h, 2 * bytes + 1024)) return NULL;
    if ((h->capacity - h->bytes) < bytes && !minimp4_vector_grow(h, bytes)) return NULL;
    assert(h->data);
...
int MP4E_add_track(MP4E_mux_t* mux, const MP4E_track_t* track_data)
{
    ...
    if (!mux || !track_data) return MP4E_STATUS_BAD_ARGUMENTS;

    tr = (track_t*) minimp4_vector_alloc_tail(&mux->tracks, sizeof(track_t));
    if (!tr) return MP4E_STATUS_NO_MEMORY;
...
static int append_mem(minimp4_vector_t* v, const void* mem, int bytes)
{
    ...
    for (i = 0; i + 2 < v->bytes;)
    {
        ...
        if (cb == bytes && !memcmp(p + i + 2, mem, cb)) return 1;
        i += 2 + cb;
    }
    size[0] = bytes >> 8;
    size[1] = bytes;
...
int MP4E_set_dsi(MP4E_mux_t* mux, int track_id, const void* dsi, int bytes)
{
    ...
    if (tr->vsps.bytes) return MP4E_STATUS_ONLY_ONE_DSI_ALLOWED;  // only one DSI allowed
 ...
 */
static int mp4e_write_fragment_header(
    MP4E_mux_t* mux,
    int         track_num,
    int         data_bytes,
    int         duration,
...
{
    track_t* tr;
    if (!mux || !data) return MP4E_STATUS_BAD_ARGUMENTS;
    ...
    {
        if (mux->sequential_mode_flag) ERR(write_pending_data(mux, tr));
        if (!add_sample_descriptor(mux, tr, data_bytes, duration, kind)) return MP4E_STATUS_NO_MEMORY;
    }
    else
    {
        if (!mux->sequential_mode_flag)
    ...
    if (mux->sequential_mode_flag)
    {
        if (!minimp4_vector_put(&tr->pending_sample, data, data_bytes)) return MP4E_STATUS_NO_MEMORY;
    }
    else
    {
        ERR(mux->write_callback(mux->write_pos, data, data_bytes, mux->token));
...
int MP4E_set_text_comment(MP4E_mux_t* mux, const char* comment)
{
    if (!mux || !comment) return MP4E_STATUS_BAD_ARGUMENTS;
    if (mux->text_comment) free(mux->text_comment);
...
#define TRACK_HEADER_BYTES 512
    ...
    if (mux->text_comment) index_bytes += 128 + strlen(mux->text_comment);
    ...
    if (!base) return MP4E_STATUS_NO_MEMORY;
    p = base;
...
#define MOOV_TIMESCALE 1000
    ...
    for (ntr = 0; ntr < ntracks; ntr++)
    {
        ...
        {
            int lang_code =
                ((tr->info.language[0] & 31) << 10) | ((tr->info.language[1] & 31) << 5) | (tr->info.language[2] & 31);
        ...
        // name is a null-terminated string in UTF-8 characters which gives a human-readable name for the track type
        // (for debugging and inspection purposes). set mdia hdlr name field to what quicktime uses. Sony smartphone may
        // fail to decode short files w/o handler name
        if (handler_ascii)
        {
        ...
        // If the flag is set indicating that the data is in the same file as this box, then no string (not even an
        // empty one) shall be supplied in the entry field.
...
#define WRITE_OD_LEN(size)   \
    ...
    WRITE_1(size)
                ...
                WRITE_3(
                    tr->info.u.a.channelcount * 6144 / 8);  // bufferSizeDB in bytes, constant as in reference decoder
        ...
        // Chunk Offset Box
        int is_64_bit = 0;
        if (samples_count && sample[samples_count - 1].offset > 0xffffffff) is_64_bit = 1;
        if (!is_64_bit)
        {
...
#define MP4E_HANDLER_TYPE_MDIR 0x6d646972
        ...
        WRITE_4(0);
        WRITE_4(0);  // reserved[3]
        WRITE_4(0);  // name is a null-terminated string in UTF-8 characters which gives a human-readable name for the
...
int MP4E_close(MP4E_mux_t* mux)
{
    ...
    if (!mux) return MP4E_STATUS_BAD_ARGUMENTS;
    if (!mux->enable_fragmentation) err = mp4e_flush_index(mux);
    if (mux->text_comment) free(mux->text_comment);
...
#define SWAP32(x) \
    (uint32_t)((((x) >> 24) & 0xFF) | (((x) >> 8) & 0xFF00) | (((x) << 8) & 0xFF0000) | ((x & 0xFF) << 24))
...
static int find_mem_cache(void* cache[], int cache_bytes[], int cache_size, void* mem, int bytes)
{
    ...
    {
        if (cache_bytes[i] == bytes && !memcmp(mem, cache[i], bytes)) return i;  // found
    }
    for (i = 0; i < cache_size; i++)
    {
        if (!cache_bytes[i])
...
void mp4_h26x_write_close(mp4_h26x_writer_t* h)
{
    ...
    {
        if (p->sps_cache[i]) free(p->sps_cache[i]);
    }
    for (i = 0; i < MINIMP4_MAX_PPS; i++)
    {
        if (p->pps_cache[i]) free(p->pps_cache[i]);
    }
#endif
    memset(h, 0, sizeof(*h));
}

static int mp4_h265_write_nal(
    mp4_h26x_writer_t* h, const unsigned char* nal, int sizeof_nal, unsigned timeStamp90kHz_next)
{
    ...
    if (is_intra && !h->need_sps && !h->need_pps && !h->need_vps) h->need_idr = 0;
    switch (payload_type)
    {
        case HEVC_NAL_VPS:
        ...
        default:
            ...
            }
            {
                ...
                if (!tmp) return MP4E_STATUS_NO_MEMORY;
                ...
                if (is_intra) sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
...
int mp4_h26x_write_nal(mp4_h26x_writer_t* h, const unsigned char* nal, int length, unsigned timeStamp90kHz_next)
{
    ...
    for (;; nal++)
    {
        ...
        if (9 == payload_type) continue;  // access unit delimiter, nothing to be done
#if MINIMP4_TRANSCODE_SPS_ID
        ...
        if (!nal1) return MP4E_STATUS_NO_MEMORY;
        ...
        switch (payload_type)
        {
            case 7:
                ...
                break;
            case 8:
                if (h->need_sps) goto exit_with_free;
        ...
        switch (payload_type)
        {
            ...
            case 8:
                ...
                break;
            case 5:
                if (h->need_sps) return MP4E_STATUS_BAD_ARGUMENTS;
                h->need_idr = 0;
                // flow through
            default:
                if (h->need_sps) return MP4E_STATUS_BAD_ARGUMENTS;
...
{
    uint8_t c;
    if (mp4->read_callback(mp4->read_pos, &c, 1, mp4->token)) return -1;
    mp4->read_pos++;
    return c;
}
...
{
    mp4->read_pos += pos;
    if (mp4->read_pos >= mp4->read_size) *eof_flag = 1;
}
...
#if MP4D_TRACE_SUPPORTED
        box_path[depth] =
            (box_name >> 24) | (box_name << 24) | ((box_name >> 8) & 0x0000FF00) | ((box_name << 8) & 0x00FF0000);
        TRACE(
        ...
        switch (box_name)
        {
            ...
            case OD_DSI:     // ISO/IEC 14496-1 Page 28. Section 8.6.5 - DecoderConfigDescriptor.
                ...
                if (!tr->dsi && payload_bytes)
                {
                    ...
                    {
                        tr->dsi[i] = minimp4_read(mp4, 1,
                                                  &eof_flag);  // These bytes available due to check above
                    }
                    tr->dsi_bytes = i;
                    payload_bytes -= i;
                    break;
    ...
    const MP4D_demux_t* mp4, unsigned int ntrack, int pps_flag, int nsps, int* sps_bytes)
{
    ...
    if (ntrack >= mp4->track_count) return NULL;
    ...
    if (pps_flag)
    {
        ...
        if (skip_bytes < 0) return NULL;
        bytepos += skip_bytes;
    }

    // Skip sps/pps before the given target
    sps_count = p[bytepos++];
    if (nsps >= sps_count) return NULL;
...
No|type|lng| duration           | bitrate| Stream type            | Object type
 0|mdir|und|  92.42 s   3980 frm|  128000| AudioStream            | Audio ISO/IEC 14496-3MP4 FILE: 1 tracks found. Movie
time 92.42 sec
